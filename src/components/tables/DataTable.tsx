          ...
          <thead>
            <tr className="border-b border-bg-border bg-white/[0.02]">
              {columns.map((c) => (
                <th
                  key={c.key}
                  className="text-left px-6 py-4 text-xs uppercase tracking-widest text-muted font-medium"
                >
                  {c.label}
                </th>
              ))}
          ...
          <tbody>
            {data.length === 0 ? (
              ...
              data.map((row, i) => (
                <tr
                  key={i}
                  className="border-b border-bg-border/50 hover:bg-white/[0.03] transition"
                >
                  {columns.map((c) => (
